...
const SuratDetailModal: React.FC<SuratDetailModalProps> = (props) => {
    ...
    const handleSignWithQR = () => {
         props.onTambahTandaTangan(surat.id);
         setIsSigning(false);
    }
    ...
    const renderActionButtons = () => {
        ...
        if (isSuratKeluar && (surat.status === 'Draf' || surat.status === 'Revisi') && surat.pembuat.id === props.currentUser.id && props.onKirimUntukPersetujuan) {
            buttons.push(<button key="kirim" onClick={() => props.onKirimUntukPersetujuan!(surat.id)} className="flex items-center bg-slate-700 text-white px-3 py-1.5 rounded-md hover:bg-slate-800 text-sm font-medium"><SparklesIcon className="w-4 h-4 mr-2" /> Kirim untuk Persetujuan</button>);
        }
        buttons.push(<button key="arsip" onClick={props.onArchive} className="flex items-center bg-emerald-100 text-emerald-800 px-3 py-1.5 rounded-md hover:bg-emerald-200 text-sm font-medium"><ArchiveIcon className="w-4 h-4 mr-2" /> Arsipkan</button>);
        buttons.push(<button key="print" onClick={() => setPrintModalOpen(true)} className="flex items-center bg-slate-100 text-slate-800 px-3 py-1.5 rounded-md hover:bg-slate-200 text-sm font-medium"><PrinterIcon className="w-4 h-4 mr-2" /> Cetak Surat</button>);
        if (isSuratMasuk) {
             buttons.push(<button key="print-disposisi" onClick={() => setDisposisiPrintModalOpen(true)} className="flex items-center bg-slate-100 text-slate-800 px-3 py-1.5 rounded-md hover:bg-slate-200 text-sm font-medium"><PrinterIcon className="w-4 h-4 mr-2" /> Cetak Disposisi</button>);
             buttons.push(<button key="reply" onClick={() => { props.onReplyWithAI(surat as SuratMasuk); onClose(); }} className="flex items-center bg-slate-700 text-white px-3 py-1.5 rounded-md hover:bg-slate-800 text-sm font-medium"><SparklesIcon className="w-4 h-4 mr-2" /> Balas dengan AI</button>);
        }
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                 ...
                 <button onClick={() => setShowForm(!showForm)} className="flex items-center bg-slate-700 text-white px-3 py-1.5 rounded-md hover:bg-slate-800 text-sm font-medium"><PlusIcon className="w-4 h-4 mr-2" /> {showForm ? 'Batal' : 'Tambah Disposisi'}</button>
            </div>
            {showForm && (
                <form onSubmit={handleSubmit} className="p-4 bg-slate-50 border rounded-lg space-y-3">
                    ...
                    <div className="text-right"><button type="submit" className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-slate-700 hover:bg-slate-800">Kirim Disposisi</button></div>
                </form>
            )}
            <div className="space-y-4">
                {disposisiList.length > 0 ? disposisiList.map(d => (
                    <div key={d.id} className="p-4 border rounded-lg bg-white">
                        <div className="flex justify-between items-start">
                             <div><p className="font-semibold text-slate-800">Dari: {d.pembuat.nama}</p><p className="text-sm text-slate-600">Kepada: {d.tujuan.nama}</p><p className="text-xs text-slate-400 mt-1">{new Date(d.tanggal).toLocaleString('id-ID')}</p></div>
                        ...
                        {d.tujuan.id === currentUser.id && d.status === StatusDisposisi.DIPROSES && (<div className="mt-3 border-t pt-3 flex items-center justify-end space-x-2"><span className="text-sm font-medium text-slate-700">Ubah Status:</span><button onClick={() => onUpdateDisposisiStatus(surat.id, d.id, StatusDisposisi.SELESAI)} className="text-sm bg-emerald-100 text-emerald-800 px-3 py-1 rounded-md hover:bg-emerald-200">Selesai</button><button onClick={() => onUpdateDisposisiStatus(surat.id, d.id, StatusDisposisi.DITOLAK)} className="text-sm bg-red-100 text-red-800 px-3 py-1 rounded-md hover:bg-red-200">Tolak</button></div>)}
                         <div className="mt-3 border-t pt-2"><p className="text-xs font-semibold text-slate-500">Status: <span className="font-bold text-slate-800">{d.status}</span></p></div>
                    </div>
...
const ApprovalAndHistorySection: React.FC<SuratDetailModalProps> = ({ surat, currentUser, onPersetujuan }) => {
    ...
    const handleDecision = (decision: 'Disetujui' | 'Ditolak') => {
        ...
        if (decision === 'Ditolak' && !notes.trim()) { alert('Catatan wajib diisi jika menolak persetujuan.'); return; }
    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <div className="flow-root"><ul className="-mb-8">{s.approvalChain.map((step, stepIdx) => (<li key={step.id}><div className="relative pb-8">{stepIdx !== s.approvalChain.length - 1 ? (<span className="absolute top-4 left-4 -ml-px h-full w-0.5 bg-slate-200" aria-hidden="true" />) : null}<div className="relative flex items-start space-x-3">{getStatusIcon(step.status)}<div className="min-w-0 flex-1"><div className="text-sm"><p className="font-semibold text-slate-800">{step.approver.nama}</p><p className="text-slate-500">{step.approver.jabatan}</p></div>{step.timestamp && (<p className="mt-0.5 text-xs text-slate-400">{new Date(step.timestamp).toLocaleString('id-ID')}</p>)}{step.notes && (<div className="mt-2 text-sm bg-slate-50 p-2 rounded-md border"><p className="text-slate-600">{step.notes}</p></div>)}</div></div></div></li>))}</ul></div>
                {isCurrentUserApprover && s.status === 'Menunggu Persetujuan' && (<div className="mt-6 border-t pt-4"><h4 className="font-semibold text-slate-800 mb-2">Tindakan Persetujuan</h4><div><label htmlFor="approval_notes" className="block text-sm font-medium text-slate-700">Catatan (Wajib jika menolak)</label><textarea id="approval_notes" rows={3} value={notes} onChange={(e) => setNotes(e.target.value)} className="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="Berikan alasan atau instruksi revisi..." /></div><div className="mt-2 flex justify-end space-x-2"><button onClick={() => handleDecision('Ditolak')} className="px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md shadow-sm hover:bg-red-700">Tolak & Kembalikan</button><button onClick={() => handleDecision('Disetujui')} className="px-4 py-2 text-sm font-medium text-white bg-emerald-600 border border-transparent rounded-md shadow-sm hover:bg-emerald-700">Setujui & Lanjutkan</button></div></div>)}
            </div>
             {s.history.length > 0 && (<div className="border-t pt-4"><h4 className="font-semibold text-slate-800 mb-2">Riwayat Versi</h4><ul className="space-y-1 text-sm list-disc list-inside text-slate-500">{s.history.map((h, i) => (<li key={i}><span className="font-semibold text-slate-700">Versi {h.version}:</span> Direvisi pada {h.tanggal ? new Date(h.tanggal).toLocaleDateString() : '-'}</li>))}</ul></div>)}
        </div>
    )
}
...
const KomentarSection: React.FC<SuratDetailModalProps> = ({ surat, onAddKomentar, currentUser }) => {
    ...
    const sortedKomentar = [...surat.komentar].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (teks.trim()) { onAddKomentar(surat.id, teks.trim()); setTeks(''); }
    };
    return (
        <div className="space-y-4">
            <h4 className="font-semibold text-slate-800">Komentar & Diskusi Internal</h4>
            <form onSubmit={handleSubmit} className="space-y-2">
                <div><label htmlFor="komentar-teks" className="sr-only">Tambah Komentar</label><textarea id="komentar-teks" value={teks} onChange={e => setTeks(e.target.value)} rows={3} placeholder="Tulis komentar atau pertanyaan di sini..." className="block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" /></div>
                <div className="text-right"><button type="submit" className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-slate-700 hover:bg-slate-800">Kirim Komentar</button></div>
            </form>
            <div className="space-y-4 pt-4 border-t max-h-64 overflow-y-auto pr-2">
                {sortedKomentar.length > 0 ? sortedKomentar.map(k => (<div key={k.id} className="flex items-start space-x-3"><div className="flex-shrink-0 h-10 w-10 rounded-full bg-slate-200 flex items-center justify-center"><span className="text-slate-600 font-bold">{k.user.nama.charAt(0)}</span></div><div className="flex-1 bg-slate-50 p-3 rounded-lg"><div className="flex justify-between items-center"><p className="text-sm font-semibold text-slate-800">{k.user.nama}</p><p className="text-xs text-slate-500">{new Date(k.timestamp).toLocaleString('id-ID')}</p></div><p className="mt-1 text-sm text-slate-700 whitespace-pre-wrap">{k.teks}</p></div></div>)) : <p className="text-center text-sm text-slate-500 p-4">Belum ada komentar.</p>}
            </div>
        </div>
    );
};
...
const TugasSection: React.FC<SuratDetailModalProps> = ({ surat, allUsers, currentUser, onAddTask }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (deskripsi && tujuanId && tanggalJatuhTempo) {
            ...
            onAddTask(newTask);
            setShowForm(false); setDeskripsi(''); setTujuanId(''); setTanggalJatuhTempo('');
        }
    };
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                ...
                <button onClick={() => setShowForm(!showForm)} className="flex items-center bg-slate-700 text-white px-3 py-1.5 rounded-md hover:bg-slate-800 text-sm font-medium"><PlusIcon className="w-4 h-4 mr-2" /> {showForm ? 'Batal' : 'Tambah Tugas'}</button>
            </div>
            {showForm && (
                <form onSubmit={handleSubmit} className="p-4 bg-slate-50 border rounded-lg space-y-3">
                    <div><label className="block text-sm font-medium text-slate-700">Deskripsi Tugas</label><textarea value={deskripsi} onChange={e => setDeskripsi(e.target.value)} required rows={2} className="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"></textarea></div>
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className="block text-sm font-medium text-slate-700">Tugaskan Kepada</label><select value={tujuanId} onChange={e => setTujuanId(e.target.value)} required className="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"><option value="">Pilih Pengguna</option>{allUsers.map(u => <option key={u.id} value={u.id}>{u.nama}</option>)}</select></div>
                        <div><label className="block text-sm font-medium text-slate-700">Tanggal Jatuh Tempo</label><input type="date" value={tanggalJatuhTempo} onChange={e => setTanggalJatuhTempo(e.target.value)} required className="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"/></div>
                    </div>
                    <div className="text-right"><button type="submit" className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-slate-700 hover:bg-slate-800">Simpan Tugas</button></div>
                </form>
            )}
            <div className="space-y-2">
                {surat.tugasTerkait.length > 0 ? surat.tugasTerkait.map(t => (
                    <div key={t.id} className="p-3 border rounded-lg flex justify-between items-center">
                        <div><p className="font-medium">{t.deskripsi}</p><p className="text-xs text-slate-500">Untuk: {t.ditugaskanKepada.nama} | Jatuh Tempo: {new Date(t.tanggalJatuhTempo).toLocaleDateString()}</p></div>
...
const InfoItem: React.FC<{ label: string; value: string }> = ({ label, value }) => (<div><p className="font-semibold text-slate-800">{label}</p><div className="text-slate-600" dangerouslySetInnerHTML={{ __html: value }}></div></div>);
const TabButton: React.FC<{ label: string; isActive: boolean; onClick: () => void, disabled?: boolean }> = ({ label, isActive, onClick, disabled }) => (<button onClick={onClick} disabled={disabled} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${isActive ? 'border-slate-700 text-slate-800' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} disabled:text-gray-300 disabled:hover:border-transparent disabled:cursor-not-allowed`}>{label}</button>);
